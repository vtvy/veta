...
const UserController = {
  ...
  follow: async (req, res) => {
    ...
    try {
      var myself = await User.findOne({ user: userID }).select("following");
      var friend = await User.findOne({ user: friendID }).select("followers");
      const notFollow = friend && !myself.following.includes(friendID);
      var state = 0;
      if (notFollow) {
        //Follow
